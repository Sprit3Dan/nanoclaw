...
class MessageTool(Tool):
    ...
    async def execute(
        ...
        **kwargs: Any
        ...
        # A2A: pin reply to the originating agent unless explicitly overridden.
        if channel == "a2a":
            metadata = self._default_metadata.get()
            a2a_ctx = metadata.get("_a2a") if isinstance(metadata, dict) else {}
            a2a_ctx = a2a_ctx if isinstance(a2a_ctx, dict) else {}
            pinned_chat_id = str(a2a_ctx.get("from_agent") or "").strip()
            allow_override_raw = kwargs.get("allow_a2a_chat_override") or (
                metadata.get("allow_a2a_chat_override") if isinstance(metadata, dict) else None
            )
            allow_override = allow_override_raw is True or (
