...
class AgentLoop:
    ...
    def _register_default_tools(self) -> None:
        ...
        if self._vision_provider:
            self.tools.register(VisionAnalyzeTool(
                vl_provider=self._vision_provider,
                vl_model=self._vision_model or self._vision_provider.get_default_model(),
                context_updates=self._vision_context_updates,
            ))
    ...
    async def _process_message(
        ...
        # A2A: if this message is a delegation completion, forward to reply target.
        if msg.channel == "a2a":
            active_delegation = self.bus.delegation_map.resolve(msg.metadata or {})
            if active_delegation is not None:
                if active_delegation.id:
                ...
                return OutboundMessage(
                    channel=active_delegation.reply_channel,
                    chat_id=active_delegation.reply_chat_id,
    ...
    def _persist_usage(self, session: Session) -> None:
        """Write LLM-reported token counts into session metadata for consolidation."""
