...
class Consolidator:
    ...
    def observed_prompt_tokens(self, session: Session) -> tuple[int, str]:
        """Return actual prompt tokens reported by the LLM, or 0 if unavailable.

        Prefer LLM-reported counts over estimation — vLLM and other providers
        only return usage when stream_options.include_usage=True is set, so
        this may return 0 for providers that don't support it.
        """
