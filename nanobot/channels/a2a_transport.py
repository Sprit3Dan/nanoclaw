...
class A2ATransport(ABC):
    @abstractmethod
    async def subscribe(self, agent_id: str, on_envelope: OnEnvelope) -> None:
        """Declare this agent's channel and start consuming. Returns when stop() is called."""

    @abstractmethod
    async def publish(self, to_agent: str, envelope: Envelope) -> None:
        """Deliver envelope to to_agent's channel."""
